import { AgronomicRule } from "../types/recommendation";

export function validateAgronomicRule(rule: unknown): { valid: boolean; errors: string[] } {
  const errors: string[] = [];
  if (!rule || typeof rule !== "object") {
    return { valid: false, errors: ["AgronomicRule must be a non-null object"] };
  }

  const r = rule as Partial<AgronomicRule>;

  if (!r.id) errors.push("Missing rule id");
  if (!r.version) errors.push("Missing rule version");
  if (!r.condition) errors.push("Missing rule condition");
